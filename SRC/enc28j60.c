...
/*************************************************************************************
FunctionName:  u8 ENC28J60_Init(void)
Author  		:  LFT
Date    		:	 20170419
Description	:  ENC28J60初始化，0,初始化成功;1,初始化失败;
History			:  历史修改记录
...
/**********************************************************************************
FunctionName:  u8 ENC28J60_Read_Op(u8 op,u8 addr)//读取ENC28J60控制寄存器(带操作码)
Author  		:  LFT
Date    		:	 20170419
Description	:  op：操作码；addr:寄存器地址/参数；返回值:读到的数据
不能直接读取ENC28J60数据，需要确定该数据寄存器在哪个Bank
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_Write_Op(u8 op,u8 addr,u8 data)//向ENC28J60寄存器写入数据(带操作码)
Author  		:  LFT
Date    		:	 20170419
Description	:  op：操作码；addr:寄存器地址；data:参数
不能直接写数据到ENC28J60，需要确定该数据寄存器在哪个Bank
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_Read_Buf(u32 len,u8* data)//读取ENC28J60接收缓存数据
Author  		:  LFT
Date    		:	 20170419
Description	:  len:要读取的数据长度；data:输出数据缓存区(末尾自动添加结束符)
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_Write_Buf(u32 len,u8* data)//向ENC28J60写发送缓存数据
Author  		:  LFT
Date    		:	 20170419
Description	:  len:要写入的数据长度；data:数据缓存区
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_Set_Bank(u8 bank)//设置ENC28J60寄存器Bank
Author  		:  LFT
Date    		:	 20170419
Description	:  ban:要设置的bank
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  u8 ENC28J60_Read(u8 addr)//读取ENC28J60指定寄存器 
Author  		:  LFT
Date    		:	 20170419
Description	:  addr:寄存器地址；返回值:读到的数据
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_Write(u8 addr,u8 data)//向ENC28J60指定寄存器写数据
Author  		:  LFT
Date    		:	 20170419
Description	:  addr:寄存器地址；data:要写入的数据
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_PHY_Write(u8 addr,u16 data)//向ENC28J60的PHY寄存器写入数据
Author  		:  LFT
Date    		:	 20170419
Description	:  addr:寄存器地址；data:要写入的数据
History			:  历史修改记录
...
/**********************************************************************************
FunctionName:  ENC28J60_PHY_Read(u8 addr)//读取ENC28J60的PHY寄存器
Author  		:  LFT
Date    		:	 20170419
Description	:  addr:要读取的寄存器地址；返回值:读取到的PHY的值
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_Get_EREVID(void)//读取ENC28J60的版本号
Author  		:  LFT
Date    		:	 20170419
Description	:  返回值:ENC28J60版本号
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_Get_Duplex(void)//获取ENC28J60双工状态
Author  		:  LFT
Date    		:	 20170419
Description	:  返回值:1，全双工；0，半双工
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName:  ENC28J60_Packet_Send(u32 len,u8* packet)//通过ENC28J60发送数据包到网络
Author  		:  LFT
Date    		:	 20170419
Description	:  len:数据包大小；packet:数据包
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName: ENC28J60_Packet_Receive(u32 maxlen,u8* packet)//从网络获取一个数据包内容
Author  		:  LFT
Date    		:	 20170419
Description	:  maxlen:数据包最大允许接收长度；packet:数据包缓存区；返回值:收到的数据包长度(字节)
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName: ENC28J60_ISRHandler(void)//中断处理函数
Author  		:  LFT
Date    		:	 20170419
Description	:  实际上不是中断函数，而是在INT外部中断函数里面调用
History			:  历史修改记录
***********************************************************************************/
...
/**********************************************************************************
FunctionName: EXTI1_IRQHandler(void)//外部中断线1的中断服务函数
Author  		:  LFT
Date    		:	 20170419
Description	:  
History			:  历史修改记录
