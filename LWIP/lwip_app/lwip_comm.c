...
//bit4~0:保留
u8 tcp_flag;
u8 tcp_recvbuf[TCP_RX_BUFSIZE];								 //TCP Client接收数据缓冲区
...
/*************************************************************************************
FunctionName: u8 lwip_comm_mem_malloc(void)//lwip中mem和memp的内存申请
Author  		:  
Date    		:	
Description	:  返回值:0,成功;其他,失败
History			:  历史修改记录
*************************************************************************************/
...
/*************************************************************************************
FunctionName: void lwip_comm_mem_free(void)//lwip中mem和memp内存释放
Author  		:  
Date    		:	
Description	:  返回值:0,成功;其他,失败
History			:  历史修改记录
...
/*************************************************************************************
FunctionName: void lwip_comm_default_ip_set(__lwip_dev *lwipx)//lwip 默认远端IP设置
Author  		:  
Date    		:	
Description	: lwipx:lwip控制结构体指针
History			: 历史修改记录//默认远端IP为:192.168.1.100
*************************************************************************************/
...
/*************************************************************************************
FunctionName: u8 lwip_comm_init(void)//LWIP初始化(LWIP启动的时候使用)
Author  		:  
Date    		:	
Description	: 返回值:0,成功；1,内存错误；2,DM9000初始化失败；3,网卡添加失败.
History			: 历史修改记录
*************************************************************************************/
...
/*************************************************************************************
FunctionName: void lwip_pkt_handle(void)//从网络缓冲区中读取接收到的数据包并将其发送给LWIP处理
Author  		:  
Date    		:	
Description	: 当接收到数据后调用
History			: 历史修改记录
*************************************************************************************/
...
/*************************************************************************************
FunctionName: void lwip_periodic_handle()//LWIP周期轮询任务
Author  		:  
Date    		:	
Description	: 
History			: 历史修改记录
...
/*************************************************************************************
FunctionName: void lwip_dhcp_process_handle(void)//DHCP处理任务
Author  		:  
Date    		:	
Description	: 
History			: 历史修改记录
