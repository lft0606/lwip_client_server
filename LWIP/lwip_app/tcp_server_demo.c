...
void tcp_server_App(void)
{
	...
	while(res==0)
	{
		...
		if(tcp_flag&1<<6)//是否收到数据?
		{
			...
			LCD_ShowString(30,210,lcddev.width-30,lcddev.height-210,16,tcp_recvbuf);//显示接收到的数据	
...
err_t tcp_server_poll(void *arg, struct tcp_pcb *tpcb)
{
	...
	if(es!=NULL)
	{
		if(tcp_flag&(1<<7))	//判断是否有数据要发送
		{
			...
			pbuf_take(es->p,(char*)tcp_sendbuf,strlen((char*)tcp_sendbuf));
			tcp_server_senddata(tpcb,es); 		//轮询的时候发送要发送的数据
			tcp_flag&=~(1<<7);  			//清除数据发送标志位
